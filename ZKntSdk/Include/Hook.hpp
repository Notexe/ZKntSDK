...
template<class T> class HookResult {
  public:
    HookResult(HookAction::Return, T p_ReturnVal) : m_ReturnVal(p_ReturnVal), m_HasReturnVal(true) {}
...
template<class ReturnType, class... Args> class Hook<ReturnType(Args...)> : public HookBase {
  public:
...
template<class... Args> class Hook<void(Args...)> : public HookBase {
  public:
...
#define DECLARE_DETOUR_WITH_CONTEXT(ContextType, ReturnType, DetourName, ...)                    \
    template<class... Args> static HookResult<ReturnType> DetourName(void* th, Args... p_Args) { \
...
#define DECLARE_STATIC_DETOUR(ReturnType, DetourName, ...) \
    static HookResult<ReturnType> __fastcall DetourName(void*, Hook<ReturnType(__VA_ARGS__)>* p_Hook, __VA_ARGS__);

#define DEFINE_STATIC_DETOUR(ParentType, ReturnType, DetourName, ...) \
    HookResult<ReturnType> __fastcall ParentType::DetourName(void*, Hook<ReturnType(__VA_ARGS__)>* p_Hook, __VA_ARGS__)
