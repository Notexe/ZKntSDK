...
template<typename T> class TBinaryTreeIterator : public TIterator<T> {
  public:
    TBinaryTreeIterator() : TIterator<T>(nullptr) {}

    explicit TBinaryTreeIterator(T* ptr) : TIterator<T>(ptr) {}

    TBinaryTreeIterator<T>& operator++() {
        using NodeType = TBinaryTreeNode<std::remove_const_t<T>>;

        const NodeType* s_Node = reinterpret_cast<const NodeType*>(reinterpret_cast<const char*>(this->m_pCurrent) - offsetof(NodeType, m_data));
